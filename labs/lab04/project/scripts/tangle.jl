# # Генератор отчётов из Literate-скриптов
# 
# Данный скрипт преобразует литературные Julia-файлы в различные форматы.
...
function main()
    if length(ARGS) == 0
        ...
        Примеры:
          julia tangle.jl scripts/sir_run_basic.jl
          julia tangle.jl scripts/sir_scan_beta.jl
        """)
        return
    end
